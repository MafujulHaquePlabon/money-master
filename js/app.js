...
const foodInputFieldValue= parseFloat(foodInputField.value) ;
console.log(foodInputFieldValue);
const rentInputField=document.getElementById("rent-input");
const rentInputFieldValue= parseFloat(rentInputField.value) ;
console.log(rentInputFieldValue);
const clothesInputField=document.getElementById("clothes-input");
const clothesInputFieldValue= parseFloat(clothesInputField.value) ;
console.log(clothesInputFieldValue); 
...
const totalExpensesText=totalExpenses.innerText;
const previousTotalExpensesAmount= parseFloat(totalExpensesText);
const  newExpensesTotal= foodInputFieldValue + rentInputFieldValue + clothesInputFieldValue;
const expensesTotal= newExpensesTotal + previousTotalExpensesAmount;
totalExpenses.innerText=expensesTotal;
 ...
 const balanceTotalAmount=balanceAmount+incomeInputFieldValue;
 const restBalanceTotal= balanceTotalAmount - expensesTotal;
 balance.innerText= restBalanceTotal;
});
...
const saving=document.getElementById("saving-amount");
const previousSavingAmount=parseFloat(saving.innerText);
//console.log(previousSavingAmount);
// Repite hyse ay  2 line
const incomeInputField=document.getElementById("income-input");
const incomeInputFieldValue=parseFloat(incomeInputField.value);

const newSavingAmount=  incomeInputFieldValue * saveInputValue/100;
const totalSavingAmount= previousSavingAmount + newSavingAmount;
...
const remainingBalance=document.getElementById("remaining-balance");
const previousRemainingBalance= parseFloat(remainingBalance.innerText);
console.log(previousRemainingBalance);

// rest balance call kore ante hbe
const newRemainingBalance=restBalanceTotal-totalSavingAmount;
